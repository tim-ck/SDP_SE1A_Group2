    ...
    public partial class RentShowcase : Form
    {
        ...
        private void btnRent_Click(object sender, EventArgs e)
        {
            using (var db = new sdpEntities())
            {
                ...
                string newID = (!db.rentinfo.Any()) ? (newID = "001") : (int.Parse(db.rentinfo.Max(p => p.rentID))+1).ToString("D3");

                string sql = "Insert into rentinfo(rentID, tenantID, startDate, duration, showcaseid, storeID) values('" + newID + "', '" + TenantMain.tenantID + "', '" + DateTime.Now + "', '" + txtPeriod.Text.ToString() + "', '" + lblShowcaseId.Text.ToString() +"','"+ lblShowcaseId.Text.ToString().Substring(0,3) + "')";
